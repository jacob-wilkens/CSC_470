...
(define block-expression-parser
  (lambda (block-expression)
    (cond
      ...
      (else (cons (lst-gen (car block-expression)) (block-expression-parser (cdr block-expression)))))))
  ...
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'block)
       (list 'block-exp (block-expression-parser (cdr no-code))))
...
(define -h
  (lambda (lst env)
   (cdr (run-parsed-code lst env))))

(define run-parsed-block-exp
  (lambda (parsed-block-exp env vars)
    (cond
      ((null? parsed-block-exp) vars)
      ((eq? (caar parsed-block-exp) 'remember-exp) (run-parsed-block-exp (cdr parsed-block-exp) (run-parsed-code (car parsed-block-exp) env) vars))
      ((eq? (caar parsed-block-exp) 'spit-out-exp) (run-parsed-block-exp (cdr parsed-block-exp) env (append vars (-h (car parsed-block-exp) env))))
      (else (-h (car parsed-block-exp) env)))))
  ...
  (lambda (parsed-no-code env)
    (cond
      ((eq? (car parsed-no-code) 'block-exp)
       (run-parsed-block-exp (cadr parsed-no-code) env '()))
