...
(define body
  (lambda (lst)
    (cond
      ((null? lst) '())
      (else (cons (no-parser (car lst)) (body (cdr lst)))))))

(define merge-env
  (lambda (lst env params)
    (append env (env-variables lst env params))))

(define env-variables
  (lambda (lst env params)
    (cond
      ((null? lst) '())
      (else (cons (list(car params) (run-parsed-code(car lst) env)) (env-variables (cdr lst) env (cdr params)))))))
...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
      ...
      (else
       (cond
         ((eq? (length (cadr(cadr parsed-no-code))) 1) (run-parsed-code (cadr parsed-no-code) env))
         (else
          (run-parsed-code
           (cadr parsed-no-code)
           (merge-env (caddr parsed-no-code) env (cdr(cadr(cadr parsed-no-code)))))))))))

(define env '((age 21) (a 7) (b 5) (c 23)))
(define sample-no-code '(call (function (m n) (do-mathy-stuff + m n)) (do-mathy-stuff % a b) a))
(define p (no-parser sample-no-code))
