...
(define run-parsed-function-code
  (lambda (parsed-no-code-function curr_env updated_env)
    (cond
      ((eq? (caadr(caddr parsed-no-code-function)) 'call-exp)
       (run-parsed-code (cadr (caddr parsed-no-code-function)) curr_env))
      (else (run-parsed-code (cadr (caddr parsed-no-code-function)) updated_env)))))
...
(define sample-no-code '(call (function (a) (call (function (r) b) 10)) 5))
(define parsed-no-code (no-parser sample-no-code))
(define t (list 'func-exp (list 'params 'a) (list 'body (list 'call-exp (list 'func-exp (list 'params 'r) (list 'body (list 'var-exp 'a))) (list (list 'num-lit-exp 10))))))
