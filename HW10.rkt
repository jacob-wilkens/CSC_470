...
(define burnToGlobal
  (lambda (los)
    (cond
      ((eq? (caar los) 'global) (car los))
      (else (burnToGlobal (cdr los))))))
...
(define newFuncScope
  (lambda (scope env)
    (append scope (list(addScope 'temp env)))))
        
(define run-parsed-function-code
  (lambda (parsed-no-code-function scope env)
    (cond
      ((eq? (caadr(caddr parsed-no-code-function)) 'call-exp)
        (run-parsed-code (cadr (caddr parsed-no-code-function)) (newFuncScope scope env) (cadr(burnToGlobal scope))))
     (else (run-parsed-code (cadr (caddr parsed-no-code-function)) scope env)))))

(define wrapper
  (lambda (c scope)
    (run-parsed-code c scope '())));(cadr scope))))
           
(define run-parsed-code
  (lambda (parsed-no-code scope env)
    (cond
      ...
      (else
        ...
        (extend-env
         ...
         (map (lambda (packet) (run-parsed-code (car packet) scope (cadr packet))) (map (lambda (x) (list x env)) (caddr parsed-no-code)))
         env))))))

(define env '((age 21) (a 7) (b 5) (c 23)))
(define scope (list(addScope 'global env)))
(define sample-no-code '(call (function (e) (call (function (r) r) 33)) 5))
(define parsed-no-code (no-parser sample-no-code))
(wrapper parsed-no-code scope)
